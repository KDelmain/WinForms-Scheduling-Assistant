...
namespace Delmain_C969
{
    public partial class AddAppointment : Form
    {
        ...
        private void addAppointment()
        {
            ...
            MySqlCommand cmd = new MySqlCommand($"INSERT INTO appointment VALUES ('{appID}', '{custIDTB.Text}', '{User.UserID}', '{"not needed"}', '{"not needed"}', '{"not needed"}', '{"not needed"}', '{typeTB.Text}', '{"not needed"}', '{DateTimeHandler.toUTCTime(startPicker.Text)}', '{DateTimeHandler.toUTCTime(endPicker.Text)}', '{DateTimeHandler.timestamp()}', '{User.Username}', '{DateTimeHandler.timestamp()}', '{User.Username}' )", con);
